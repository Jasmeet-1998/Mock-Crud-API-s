...
- Open a terminal and type npm start hit enter.
...
FOR TEST
-------------------
NOTE-> IF YOU ARE TESTING THE API AFTER BUILDING DOCKER IMAGE GO TO THE CONSTANTS DIRECTORY AND REPLACE 3000 WITH 8080 PORT IN THE tests.js as the Dockerfile config has exposed port 8080.

For local testing at port 3000 i.e you are not running the server and test via docker image then follow below instructions.

-Configure the viewloans.js in the test directory a/c to the credentials of the user you make using the register route then open a new terminal and Run npm test on different terminal while the server is also running in different terminal.
